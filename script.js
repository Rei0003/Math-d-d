...
const gameHeader = document.querySelector('.game-container header');
const gameMain = document.querySelector('.grid-layout');

const btnStartGame = document.getElementById('btn-start-game');
const selectDiffBtns = document.querySelectorAll('.select-diff-btn');
const selectCharBtns = document.querySelectorAll('.select-char-btn');
...
function setupEnemyForLevel(level) {
    let baseEnemy = enemyList[level - 1];
    let finalHp = baseEnemy.hp;

    if (difficulty === "easy") {
        finalHp = Math.floor(baseEnemy.hp * 0.5);
    }
...
function attack() {
    ...
    let baseDamage = (player.stats.Spēks * 1.0) + (player.stats.Veiklība * 0.4) + (player.stats.Intelekts * 0.4);
    let damage = Math.floor(baseDamage) + Math.floor(Math.random() * 5);
...
function gameWon() {
    ...
    updateUI();
    logMsg(`<b>EPISKA UZVARA!</b> Tu esi pieveicis Haosa Lordsu un izgājis visus 10 līmeņus. Tavs intelekts un disciplīna ir nepārspējami!`, "gold");
    endGame();
}
...
function restartGame() {
    ...
    if (btnPotion) btnPotion.classList.remove('hidden');
...
if (btnPotion) btnPotion.addEventListener('click', drinkPotion);
...
selectCharBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        if (race === 'elf') {
            player.maxHp = 40;
            player.hp = 40;
            player.stats = { Spēks: 4, Veiklība: 8, Konstitūcija: 4, Intelekts: 8, Gudrība: 6, Harizma: 6 };
        } else if (race === 'human') {
            player.maxHp = 50;
            player.hp = 50;
            ...
            player.maxHp = 65;
            player.hp = 65;
            player.stats = { Spēks: 8, Veiklība: 3, Konstitūcija: 7, Intelekts: 3, Gudrība: 4, Harizma: 4 };
        }
        ...
        const heroNameText = card.querySelector('h3').innerText;
        const diffText = difficulty === "easy" ? "Vieglajā" : "Vidējā";

        gameLog.innerHTML = `Piedzīvojums sākas ${diffText} sarežģītībā! Kā <b>${heroNameText}</b> tu dodies katakombās. Tavā priekšā stāv ${enemy.name}!`;
